...
- Tradeable: 事件是否可交易，要求 上市满90个自然日，事件发生于交易日，非停牌，无涨停，去除特殊000792.SZ
    - 可能还需要去除instnum高的（输入event_first_report已经去除
- TradeableCount: 该日期可交易事件数
(Feb. 21st)
将（二）分组更新到面板，二分组规则：只有一个事件，则H组L组均持有；事件数为奇数，交替0101令L组/H组多一个事件，使得两组事件数平衡
- odd_adj: 奇数事件数调整（奇数日交替是/否调整，用于二分组）
- single: 该日仅有一个事件，则High组/Low组均覆盖该事件
- L_CAR_8, H_CAR_8: 事件在日截面上根据CAR_8(事前8日累计超额收益)进入Low组/High组
- L_CAR_6, H_CAR_6: 同上，根据CAR_6
- L_AR0, H_AR0: 同上，根据AR0(事件当天超额收益)
- L_AR1, H_AR1: 同上，根据AR1
- L_AR2, H_AR2: 同上，根据AR2
"""
