...
def cal_fac_ret(conf, data_pat, cache_path, panel_path, fval_path, omega_path):
    ...
    for year in range(2022, 2011, -1):
        ...
        # def check():
        #     from AllSample.template import *
        #     df = dat; msg = 'Fully-Covered Stock Numbers: Barra Exposure'
        #     df = pd.DataFrame(rtn_ctc); msg = 'Fully-Covered Stock Numbers: Next Day Return'
        #     tmp = df.dropna().iloc[:, 0]
        #     tmp.groupby(tmp.index.get_level_values(0)).count().plot(title=msg)
        #     plt.tight_layout();
        #     plt.show()
