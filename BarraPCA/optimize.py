...
for td0 in tradedates:
    ...
    # print('指成分股缺暴露的', len(stk_ind_cons.difference(stk_alpha)))
    # print(f' Alpha:{stk_alpha.__len__()}({tmp.__len__()})\t覆盖(暴露缺失)')
    ...
    # len(stk_ind_cons.difference(stk_ingredient))
    # w = wb.reindex_like(pd.Series(index=stk_ingredient)).fillna(0)
    # w /= w.sum()
...
# %%
portfolio_weight.to_csv(dat_path + f'portfolio_factor_apm_weekly[{mkt_type}].csv')
...
tmp.plot()
plt.show()
tmp.to_excel(dat_path + f'wealth_portfolio_factor_apm_weekly[{mkt_type}].xlsx')
del tmp
